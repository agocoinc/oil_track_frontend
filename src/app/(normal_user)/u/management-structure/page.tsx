...
import { fetchCompanyStructure, createStructureNode, deleteStructureNode, ValidationErrorResponse, fetchPublicManagement } from "@/lib/structure";
...
useEffect(() => {
  const init = async () => {
    ...
    try {
      // ✅ Use the new helper
      const res = await fetchPublicManagement(Number(user?.company_id));
        console.log("here =>>>", res)
      if (res.status && Array.isArray(res.data)) {
        // Normalize parent_id if needed
        const normalized = res.data.map((node) => ({
          ...node,
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col p-6 gap-4">
          <div className="flex items-center justify-between">
            ...
            <div>
            <Button className="ml-2" onClick={() => router.push('/u/structure')}>عرض المخطط</Button>
